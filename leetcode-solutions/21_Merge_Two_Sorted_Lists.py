    ...
    def mergeTwoLists(self, list1, list2):
        ...
        while list1 and list2: # When the current value of each list is not None/empty, iterate.
            if list1.val < list2.val: # if list1.val is less than list2.val, list1.val will be popped in first.
                ...
                list1 = list1.next # get the next value of the list
            else: # Reverse of the logic above
                tail.next = list2
                list2 = list2.next
            tail = tail.next # for the next iteration, set the tail to be the end again.

        # we reach the following logic if either of the list1 or the list2 are empty; in this case, what remains (either list1 or list2), can be slapped on at the end
        if list1: 
            tail.next = list1
        elif list2:
            tail.next = list2
